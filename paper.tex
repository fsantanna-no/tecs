...
more fine-grained control over program execution targeting soft real-time
systems.
%
...
for hard real-time systems~\cite{esterel.fixed}.
...
Figure~\ref{lst.syn} presents the subset of the concrete syntax of \CEU used in
...
\end{lstlisting}
\caption{ Subset of the concrete syntax of \CEU used in the paper.
\label{lst.syn}
}
\end{figure}
...
We finish the section with a summary of our design
    (\ref{sec.ceu.summary}).
%
We present the formal specification of the semantics of \CEU with a simplified
abstract syntax in a separate paper~\cite{ceu.phd}.
...
%
This approach avoids any form of graph construction or data dependency analysis
as required in Esterel, e.g.:
``set or reset of a signal must always precede any test of this signal (signal
coherence law and constructivity''~\cite{esterel.saxort}.
%
Also, these \emph{delayed awaits} avoid corner cases of instantaneous
...
\code{\_redraw\_non\_commutative} (ln. 2) and annotate it as safe (ln. 4--5) to
make the code compile successfully.
The new function name is explicit about its effect, improving code readability.
...
stack-based internal events.
Programmers can figure out which statement executes next, helping runtime 
...
We also measure how synchronous lengthy 
...
techniques also generates ``code less than $10\%$ larger the original
manually-sequenced code'' for an industrial-size
...
%
Some variants of Statecharts~\cite{statecharts} also distinguish internal from
external events~\cite{statecharts.variants}.
...
after completion of the step''~\cite{statecharts.statemate}, implying queue-based
execution.
In Stateflow, ``the receiving state (of the event) acts here as a
function''~\cite{statecharts.stateflow}, which is similar to \CEU's stack-based
execution.
To avoid recursion, \CEU adopts delayed awaits, while in Statemate, ``loops in
the broadcasting of events (between states) are forbidden''.
...
programs} that allow ``shared variables to have multiple values per tick as
long as these values are explicitly ordered by sequential
...
strict deterministic scheduling such as in \CEU.
