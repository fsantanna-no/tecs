...
blocks in sequence are packed in a \code{union}, while blocks in parallel, in a 
\code{struct}.
...
the last trail cannot awake any other trail, because they are all hanged in the 
\code{STACKED} state.
...
etc.), the compiler splits the trail in two segments with associated entry 
points.
%
...
%
This decision comes not only from the fact that \CEU targets constrained with 
...
analysis of \CEU infers precisely trails that are concurrent and yet do not 
share resources.
...
One reason is due to overhead from continuous fork-and-rejoin in small 
reactions.
Another reason is due to contention form excessive locality of data in 
stackless trails sharing contiguous static memory.
