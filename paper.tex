...
dynamic reprogramming over the air.
\end{abstract}
...
An established alternative to \emph{C} in the field of embedded systems is the 
...
%
Among the control-based languages, Esterel~\cite{esterel.ieee91} influenced a 
number of other embedded languages, such as \emph{Reactive-C}~\cite{rp.rc}, 
\emph{OSM}~\cite{wsn.osm}, \emph{Sync-C}~\cite{rp.synchc}, and 
\emph{PRET-C}~\cite{rp.pretc}.

\CEU is another language targeting embedded systems based on Esterel with some 
novel functionalities:
%
\begin{itemize}
%
\item Stack-based internal events, which provide a limited form of coroutines.
%
\item A static temporal analysis and deterministic execution semantics that 
allows programs to safely share memory.
%
\item A finalization mechanism for safe abortion of lines of execution.
%
...
In this work, we present a formal semantics of a small synchronous kernel that 
represents a subset of \CEU covering these new functionalities.
%
We also present an implementation of \CEU with two backends:
one aiming for efficiency and interoperability with \emph{C},
...
of constrained wireless sensor networks, developing a number of applications, 
protocols, and drivers.
%
We evaluated the expressiveness of \CEU in comparison to event-driven code in 
\emph{C}, and observed a reduction in source code size (around 25\%) with a 
small increase in memory usage (around 5--10\% for ROM and 
RAM)~\cite{ceu.sensys13}.
%
Considering the \emph{VM} backend, a simple application that blinks three LEDs 
periodically occupies less than 100 bytes and can be completely transmitted in 
4 radio messages.
In contrast, without the preinstalled \emph{VM} with basic \emph{I/O} 
functionality, a \emph{C}-compiled version occupies more than 2 
kbytes~\cite{ceu.terra}.
...
\CEU.
Section~\ref{sec.impl} presents the \emph{C} and \emph{VM} implementation 
backends.
...
execution policy, similar to subroutine calls in typical programming 
languages~\cite{ceu.sensys13}.
%
...
loop do
    par/or do
        await e;
    with
        emit e;     // awakes 1s trail and restarts the loop
        await f;
    end
end
\end{lstlisting}
...
avoid instantaneous termination, the \code{loop} would restart instantaneously 
if the \code{await} could awake in the same reaction that reaches it.

In atypical scenarios requiring immediate awake, delayed awaits can be 
circumvented by manually copying or making transformations in the code to 
execute on awake.
%
...
In this case, moving the \code{await} to the end of the loop makes the periodic 
code to also execute immediately (and then in reactions to each \code{emit} 
request).
If the periodic \code{emit} depends on a condition, the code becomes more 
intricate because we need to copy the condition test to the periodic code, as 
illustrated in the right of Figure~\ref{lst.delay}.
...
and stateful \emph{C} functions (e.g., device drivers) in order to preserve 
safe abortion of trails~\cite{ceu.sensys13,ceu.mod15}.
%
...
accompany the stateful \emph{C} call.
...
leaving out side effects and \emph{C} calls (which behave like in conventional 
...
The $mem(id)$ primitive represents all accesses, assignments, and \emph{C} 
function calls that affect a memory location identified by $id$.
...
%
This phase outputs code in \emph{C}, given the way \CEU is tied to \emph{C} by 
design.
%
Some type checking is delayed to the last phase to take advantage of 
\code{gcc}'s error handling.
Therefore, we annotate the \emph{C} output file with \code{\#line} pragmas 
matching the original file in \CEU.

\item[Code Generation]

The final phase packs the generated \emph{C} file with the \CEU runtime and 
...
\begin{itemize}
%
\item Nodes that represent expressions, assignments, \emph{C} calls, and 
...
reuse it.
%
Translating this idea to \emph{C} is straightforward~\cite{wsn.osm,wsn.ocram}: 
...
original version in \emph{nesC}~\cite{wsn.nesc} (a dialect of \emph{C} for 
...
\subsection{The External \emph{C} API}
...
into programs, which are all exposed as functions in a \emph{C} API:
...
In this case, the \emph{C} implementation makes a tail call to the 
corresponding handler (i.e.  \code{ceu\_go\_event} or \code{ceu\_go\_time}), as 
synchronous code has higher priority.
...
that triggers callback \code{Timer.fired} (lines 13--18) every 10 milliseconds 
and advances the wall-clock time of \CEU (line 16)%
...
programs, such as the light sensor (lines 20--24) and the radio transceiver 
(lines 26--37).

\subsection{The Terra Virtual Machine}
\label{sec.terra}

Terra is a system that implements a specif programming model for wireless 
sensor networks~\cite{Branco2015a}.
Terra is based on virtual machine concepts that uses an implementation of Céu 
language as its scripting language.

Terra programming model combines the use of configurable component-based 
virtual machines with a reactive scripting language which can be statically 
analyzed to avoid unbounded execution and memory conflicts.
This approach allows the flexibility of remotely uploading code on motes to be 
combined with a set of guarantees for the programmer.
The choice of the specific set of  components in a virtual machine 
configuration defines the abstraction level seen by the application script.

Terra uses \ceut as its scripting language and implements a component-based 
virtual machine \vmt to be customized for different application domains.
\ceut implements a variation of Céu programming language.
This variation, on one hand, excludes some of Céu's characteristics and, on the 
other hand, includes some language extensions.
We use Céu as Terra's scripting language because of its reactive nature and its 
high-level control primitives and compile-time safety guarantees.
We want only the VM components to escape the safety analysis, so we took out 
the facility to include arbitrary C code, but we did maintain all of Céu's 
original control structures.

The Terra Virtual machine \vmt, with its customizations, runs on WSN nodes with 
limited resources like MicaZ\cite{Memsic2009b} and TelosB\cite{Memsic2009} 
motes.
We built \vmt using the nesC programming language~\cite{Gay2003} and the TinyOS 
operating system~\cite{Levis2004}.
We used TinyOS, basically, because it is the operating system for WSN  most 
ported to different mote types.

Figure \ref{fig:system} presents the three basic elements of Terra -- a 
scripting language (\ceut), a set of customized pre-built components, and an 
embedded virtual-machine engine.
The \vmt runtime environment is the virtual machine combined with a set of 
customized components.

\begin{figure}[!htb]
 \centering
 \includegraphics[width=0.7\columnwidth]{fig_system}
\caption{Terra programming system basic elements.}
 \label{fig:system}
\end{figure}

A \ceut program is compiled to a bytecode file that can then be disseminated to 
the network nodes, where it is interpreted by the \vmt, which implements the 
bytecode interpreter, the execution model, the code dissemination service, and 
some specific customized components.
Figure \ref{fig.impl-t} illustrates the Terra compilation process.

\begin{figure}[!htb]
\centering
\includegraphics[width=1.0\columnwidth]{impl-t}
\caption{Terra Compilation process: from the source code in \ceut to the final 
bytecode.
\label{fig.impl-t}
}
\end{figure}

Similar to the Céu compiler process, the Terra process also has the three main phases:
Parsing, Temporal Analysis, Code Generation.
The Terra parsing phase uses the original Céu parser adding some extensions to 
parse specific \ceut elements as the data types, the data structures, and the 
configuration block.
A configuration block defines all events, functions, and data structures 
available in a specific \vmt customization.
The Terra Temporal Analysis phase  only differs in the code output. Instead of 
generating code in C, Terra generates the \vmt assembly code using the original 
Céu labels for trails entry points.
...
provides a limited form of coroutines that supports reactive statements (e.g., 
\code{await} and \code{par/or}).

The event-driven approach~\cite{sync_async.whynotthreads} is well known and 
popular in many software communities,
such as web frameworks in the client side (e.g., \emph{jQuery}~\cite{js.jquery} 
and \emph{Rx.js}~\cite{js.rx}) and server side (e.g., 
\emph{Node.js}~\cite{js.node} and \emph{SEDA}~\cite{sync_async.seda}),
GUI toolkits (e.g., \emph{Tcl/Tk}~\cite{tcl.tk} and \emph{Java 
Swing}~\cite{java.swing}),
