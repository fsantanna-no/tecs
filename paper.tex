...
state variable (line 2) and also to copy the condition test to the periodic 
code (line 13).
%
...
\code{x} (lines 5,8) can never run concurrently, because each trail segment 
reacts to a different input event (lines 4,7) and, according to the semantics 
of \CEU, cannot occur simultaneously.
%
However, the program in the right is non-deterministic, because the two 
assignments to \code{y} (lines 5,8) occur in the same reaction to input 
\code{A} (lines 4,7).
...
the other transmits the packet and waits for a confirmation (lines 9--16).
%
%
...
(line 14) is \emph{asynchronous}, handing to the radio driver a pointer to the 
lexically-scoped packet (line 12).
