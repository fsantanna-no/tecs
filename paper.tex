...
requires careful static analysis to detect and reject programs with 
...
ultimately, has to solve the errors arising from corner cases.
%
...
Unlike Esterel's semantics, this precludes programs being compiled directly and
efficiently to hardware~\cite{esterel.constructive}.
It also makes difficult to statically determine worst-case reaction times for
hard real-time systems~\cite{esterel.fixed}.
...
each possible path~\cite{ceu.sensys13,esterel.primer}.
%
...
of the following control specification:
%
...
Some features are not covered in this paper, e.g.,: pausing, asynchronous execution,
and abstractions (functions and data structures).
\label{lst.syn}
}
\end{figure}
...
\item Active trails execute until they await or terminate, one after another.
...
one.
%
Based on the synchronous hypothesis, a program takes negligible time on step 
...
%
(Usually referred to as \emph{sample-driven execution}~\cite{rp.twelve}.)
%
...
Sample-driven execution maps directly to hardware implementations and does not
...
implementations with different tick lengths, the behaviors might differ.
%
...
limited form of subroutines, as described in Figure~\ref{lst.sub}.
...
Only after this sequence does the calling trail resumes and passes the assertion 
test (ln. 10--11).
...
%
Also, unlike Esterel, this approach does not require data dependency analysis
...
\end{itemize}
%
The detection algorithm, which is described in Section~\ref{sec.impl.checks},
...
parallel) and for redrawing objects on the screen.
%
...
In typical graphical APIs, consecutive calls to \code{\_redraw} overwrite 
...
Figure~\ref{lst.delta}.a is a variation of Figure~\ref{lst.timers}.a in which
...
Figure~\ref{lst.delta}.b deals with the possible case in which a small $1us$
...
loop do
    ...
    if late > 1000 then     // I am late 1000x 1us?
        <...>               // abnormal behavior
    else
        <...>               // normal behavior
    end
...
\CEU aims to offer a simple semantics with internal determinism and
...
stack-based internal events.
Programmers can figure out which statement executes next following clear
sequential rules.
%
...
synchronization.}
Given the need for timers in embedded systems, a dedicated syntax can simplify
the development and readability of programs.
...
Our approach for synchronous and deterministic execution also leads to some
...
The compilation of \CEU programs consists of three main phases:
the \emph{parsing phase} converts the source code in \CEU to an abstract syntax 
tree (\emph{AST});
the \emph{concurrency checks phase} detects inconsistencies in programs, such
as unbounded loops and suspicious concurrent statements;
the \emph{code generation phase} converts the \emph{AST} to standard \C code 
and augments it with platform-dependent functionality (e.g., system calls) and
the runtime of \CEU, compiling everything with \emph{gcc} to generate the final 
binary.
%
An alternative implementation generates bytecode for an embedded virtual
machine, which can disseminate and install programs dynamically.
...
and are in parallel branches, and compare their sets $I$ for non-empty
intersections.
...
(ln. \ax:2--4,5--7,8--12) are packed into a \code{union} (ln. \bx:2,3,4--7), 
...
Given that trails in parallel are allocated in successive slots in the static 
vector \code{TRLS} (ln. \bx:12--15), this pass only aborts the appropriate 
trails.
...
checks of \CEU detect trails that are concurrent and yet do not share
resources.
...
single-core execution on the same system.
...
%
TinyOS provides open-source industrial-level applications and serves as basis
...
networks simulated in software~\cite{ceu.sensys13}.
%
...
The small overhead in resource usage suggests that the gains in productivity
and safety with \CEU make it a viable alternative to \C in the context of 
...
In future work, we plan to perform an equivalent thorough analysis in
...
Figure~\ref{lst.impl.tinyos}) allows applications to choose possible 
...
%
\CEU incorporates this event-triggered notion of time into the core of the
language, and is a prerequisite for the concurrency checks that enable safe
...
\CEU also relies on deterministic scheduling to preserve intra-reaction
determinism.
%
...
The \emph{SCCharts} formalism~\cite{sccharts.1} proposes \emph{sequentially
constructive programs} that allow \emph{``shared variables to have multiple
values per tick as long as these values are explicitly ordered by sequential
...
set).
This approach still restricts concurrent accesses but does not impose
strict deterministic scheduling as \CEU does.
...
The global transformation relies on a special platform statement that describes
available timers in the system (e.g., sample-driven or interrupt-driven).
...
%
In contrast, \CEU makes physical time a special input event that feeds the
runtime with an associated time to elapse, which is decremented from all
awaiting trails.
...
runtime calls whenever the program awaits on an earlier timer.
...
%
As a complement to finalization, \CEU also tracks pointers representing
resources that cross \C boundaries and forces the programmer to provide
associated finalizers.
...
also to hardware circuits~\cite{esterel.constructive,esterel.emperor,esterel.tutorial}.
...
%
The \emph{The real-time specification for Java}~\cite{java.rtsj,java.rtsj.ovm}
provides region-based memory that is not subject to garbage collection, and
threads that are not preemptable by the collector.
%
...
application domain (e.g., a button click or an expiring timer).
...
research group) has released a product based on \CEU.
