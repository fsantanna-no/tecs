...
\begin{abstract}
\CEU is a synchronous language inspired by Esterel which aims to offer a 
simpler semantics and more fine-grained control over the program execution.
%
\CEU employs an event-triggered notion of time that allows for a simple
reasoning about reactions in parallel and enables compile-time analysis to 
produce deterministic and concurrency-safe programs.
%
...
}, showing the practical aspect of our clean semantics.
...
implementations), i.e., Esterel's `$\|$' and \CEU's \code{par/and} constructs 
are equivalent.
%
...
running (step 2), it is enqueued to occur in the next reaction.
...
Esterel and \CEU use discrete logical units of time in which the occurring 
events (which are the same in all boxes) have to fit somehow:
%
\begin{itemize}
\item \code{[Box-1]}:
``Esterel with fixed-length ticks'',
within which reactions to occurring inputs have to fit~\cite{esterel.fixed}.
%
...
However, the program can only detect one occurrence of \code{E}, which is 
considered in \code{R(D+E)}.
%
\item \code{[Box-2]}:
...
\code{B} actually happen much closer to one another.
In Section~\ref{sec.ceu.timers}, after introducing internal events and 
first-class timers, we show an example that detects simultaneous button clicks.
%
...
%
To cope with deterministic intra-reaction communication, \CEU supports internal 
events which programs can \code{emit} and \code{await}.
In contrast with external events, internal events follow a stack-based 
...
\item 1st trail awakes (ln. 4), broadcasts \code{b}, and pauses.
...
%
Figure~\ref{fig.redraw}.a shows an abstract code to animate and redraw objects 
...
on lexical order.
Therefore, in Figure~\ref{fig.redraw}, we redefine the redrawing function to an 
...
The design decisions XXX, raise some important limitations as follows:
%
\begin{itemize}
\item Purely reactive execution.
    hardware
A limitation of event-triggered execution is that all program behavior must be purely
reactive, given that no code can execute in the absence of inputs. Tick-triggered execu-
tion allows for active behavior, since code can execute regulary on every tick. Although
it supports asynchronous active execution [Sant’Anna et al. 2012], we still consider
 ́ as purely reactive since its synchronous core cannot express active behavior.
\item bounded-only reactions,
\item sequential
\end{itemize}
%
We believe that a simple-tractable synchronous core
and extend it with memory-isolated parallelizabe asynchronous execution.
