...
% Title portion
\title{The Design, Semantics, and Implementation of \CEU: a Synchronous Reactive Language}
...
\emph{Code Generation} phase (as illustrated in Figure~\ref{fig.impl}), which 
here outputs assembly instructions for the \VM, instead of statements in \C.
%
...
\caption{
%{\small %\textmd{
\CEU interface (in the left) with customized \VM (in the right).
%
The routine \code{VM.out} redirects all output events to the corresponding OS 
calls (ln.  1--8).
%
