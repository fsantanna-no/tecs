...
platforms.
%
Its synchronous programming model allows for a simple reasoning about 
...
- Ceu
    - first WSN
    - other domains, such as games
    - here, only static semantics
    - uses, classes, gsoc,
...
two languages, as Esterel's `$\|$' and \CEU's \code{par/and} constructs are 
equivalent (lines 4--9).
%
For the second phrase (i.e., the reset behavior), the Esterel version uses a 
...
In the sections that follow, we discuss the four basic differences between \CEU 
and Esterel: deterministic execution for operations with side-effects
(Section~\ref{sec.ceu.det}), safe abortion for lines of execution 
(Section~\ref{sec.ceu.abrt}), first-class timers 
(Section~\ref{sec.ceu.timers}), and stack-based execution for internal events 
...
I/O and lack of hardware support for real parallelism.
...
\end{minipage}
%\rule{8.4cm}{0.37pt}
\caption{ The execution order between \code{f1} and \code{f2} is unspecified in 
Esterel, whereas in \CEU, \code{\_f1} always executes before \code{\_f2}.
{\small
%TODO.
}
\label{lst.abro}
...
The code fragments of Figure~\ref{lst.abortion} shows a corner case for 
