...
\item Active trails execute, one after the other, until they await or 
      terminate.
...
The other boxes show discrete logical units of time in which the same occurring 
events fit in different ways.
%
\begin{itemize}
\item \code{[Box-1]}:
\emph{Esterel with fixed-length ticks},
in which reactions must fit~\cite{esterel.fixed}.
%
...
%
\item \code{[Box-2]}:
\emph{Esterel with variable-length ticks}, which are adjusted to the CPU time 
the corresponding reaction takes to complete~\cite{esterel.variable}.
...
behavior in \CEU is similar to \code{Box-2} for the first two reactions 
...
discrete ticks.
For instance, in Figure~\ref{fig.ticks} (\code{box-1} and \code{box-2}), 
Esterel considers the events \code{B} and \code{C} to be simultaneous, even 
though \code{A} and \code{B} ``physically'' happen much closer to one another.
...
Reactions to multiple events in \CEU never overlap because they are atomic.
Automated mutual exclusion simplifies the reasoning about concurrency and is a 
...
For instance, the code in Figure~\ref{lst.sub} that executes after the 
\code{await inc} (ln.  4) could \code{emit} another event, creating a new level 
in the stack.
The runtime stack constitutes fine-grained micro reactions, one on top of the 
...
makes the enclosing \code{loop} non instantaneous (ln 2--9).
...
%
A number of Esterel-based synchronous languages have intra-reaction determinism 
by enforcing an arbitrary execution order for statements in multiple lines of 
execution (%
...
5, 8).
%
In the example \ax, the two assignments to \code{x} execute in reactions to 
different events \code{A} and \code{B} only, which by definition, cannot occur 
...
%
Note also that the analysis is optional and does not affect the semantics of 
the program.
...
also considers concurrent calls and accesses to external symbols in \C.
%
...
(ln. 10).
%
Since \CEU does not inspect any code in \C, it opts to complain about 
suspicious concurrent accesses between \code{\_f} and all symbols in the second 
trail.
...
conflicting pixels, which makes them non commutative and prevent the code to 
compile.
%
However, we want to rely on lexical order to always redraw the 
...
\CEU goes one step further and actually enforces the use of \code{finalize} for 
system calls that deal with pointers representing resources:
%
\begin{itemize}
...
\item If \CEU \textbf{receives} a pointer from a system call, the pointer 
...
%
However, the interaction between system clocks and programs is not 100\% 
...
%
\item \emph{Static temporal analysis for concurrent statements.}
...
\end{itemize}
%
Nonetheless, we advocate for keeping a tractable synchronous reactive core with 
support for shared memory concurrency and deterministic execution.
%
To deal with the limitations above (which is not in the scope of this paper), 
we recommend for memory-isolated parallelizable asynchronous primitives as 
separate extensions to the synchronous core~\cite{esterel.crp}.
...
In the subsections that follow, we discuss \CEU-specific implementation 
details:
temporal analysis for determinism (Section~\ref{sec.impl.analysis}),
...
input events $I$ (\emph{incoming}) that can lead to the execution of the node, 
...
\code{false} are the two clauses of the \code{if}.
%
...
for blocks in sequence are packed in a \code{union}, while blocks in parallel, 
in a \code{struct}.
...
given that their variables cannot be in scope at the same time, e.g., 
\code{MEM.a\_1} and \code{MEM.b\_2} (ln.  \bx:2,3) can safely share the same 
memory slot.
%
...
execute when scheduler runtime reaches that level.
%
A \code{FINALIZE} trail represents a hanged finalization block which is
...
%
Since the scheduler traverses all trails on every reaction, it adjusts all 
\code{seqno} to avoid overflows (actually, 2 bits to hold the \code{seqno} is 
already enough).
%
The size of \code{stack\_t} depends on the maximum depth of nested emissions 
but is bounded to the maximum number of trails:
...
to the original single-threaded version in \C~\cite{ceu.sensys13}.
...
%
Then, \code{dispatch} takes \code{TRAIL-1} and executes its registered label 
\code{And\_sub\_2} (ln. 16--21), which sets \code{TRAIL-1} to await 
\code{EVT\_A} and also halts.
...
One reason is due to continuous fork-and-rejoin overhead from small reactions.
Another reason is due to excessive locality of data from stackless trails 
...
%
However, to preserve the sequential/discrete semantics of time in \CEU, these 
calls must never interleave or parallelize execution.
...
25--36).
%
The scheduler of TinyOS is already synchronous by default and always execute 
...
time in the core of the language, which is a prerequisite for the temporal 
analysis that enables safe shared-memory concurrency.
...
\CEU is a concurrency-safe language, employing static analysis to ensure that 
