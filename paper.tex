...
it signals the application with the \code{SEND\_ACK} to acknowledge completion 
(ln. 16).
%
At any time, the client may request a retransmission (ln. 8), which terminates 
the \code{par/or} (ln. 7), aborts the ongoing transmission (ln. 15, if not 
idle), and restarts the loop (ln. 6).
%
...
and stateful output events and \C calls in order to preserve safe abortion of 
trails.
%
