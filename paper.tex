...
\emph{causality} and \emph{schizophrenia} problems, with extensive coverage in 
the 
literature~\cite{esterel.constructive,esterel.d1,esterel.d2,esterel.d3,esterel.d4,esterel.d5,esterel.d6,esterel.d7,esterel.d8}.
%
...
%
Furthermore, it also affects the capacity of the programmer's understanding, 
which, after all, has to solve the errors when facing corner cases.
%
...
threads to share memory and interact with side-effect and stateful system calls 
appropriately.
%
For instance, although Esterel supports orthogonal abortion of 
threads~\cite{esterel.preemption}, it doesn't offer a finalization mechanism to 
...
Kbytes of SRAM.)
}, showing that the peculiarities in the semantics of \CEU does not pose 
...
occupies less than 100 bytes and can be transmitted over the air in a couple of 
packets.
...
%
Also, Esterel syntactically forbids sharing memory between lines of execution 
to preserve determinism: \emph{``if a variable is written by some thread, then 
it can neither be read nor be written by concurrent 
...
termination).
%
In the example in Figure~\ref{lst.shared}.a, the two assignments to \code{x} 
...
%
However, the example in Figure~\ref{lst.shared}.b is non-deterministic, because 
...
\code{printf}).%
\footnote{\CEU assumes that all system calls perform side effects, unless they 
are annotated as ``\texttt{@pure}''.}
...
program source code.
%
Therefore, even though the example in Figure~\ref{lst.shared}.b is suspicious, 
...
Figure~\ref{lst.det} compares two syntactically equivalent code fragments in 
Esterel (a) and \CEU (b) to summarize the semantic differences regarding 
(non-)determinism.
%
...
The introductory example in Figure~\ref{lst.abro} illustrates how synchronous 
...
clause (ln. 18) executes automatically on external abortion.%
\footnote{
...
\begin{comment}
The code fragments in Figure~\ref{lst.abortion} shows a corner case regarding
...
The \emph{isBlocked} predicate in Figure~\ref{fig.isBlocked} identifies a 
...
In the sections that follow, we discuss the implementation insights related to 
the peculiarities of \CEU:
...
\emph{PRET-C}~\cite{rp.pretc} and \emph{Sync-C}~\cite{rp.synchc}.
%
