...
Figure~\ref{lst.abro} shows side-by-side the implementations in Esterel (a) and 
\CEU (b) for the following control specification:
%
...
%
In the example \code{(b)}, the two assignments to \code{y} are simultaneous 
...
\end{lstlisting}
\centering\small{(a) Definition and use of external symbols}
\end{minipage}
%
...
.
\end{lstlisting}
\centering\small{(b) Annotations for the external symbols in (a).}
\end{minipage}
%\rule{8.4cm}{0.37pt}
\caption{ The unsafe program in (a) only compiles with the annotations in (b).
\label{lst.c}
}
\end{figure}
...
With the annotations in \code{(b)}, the program in \code{(a)} compiles 
successfully.
...
The example in Figure~\ref{lst.fin} in Esterel (\code{(a)}) and \CEU 
(\code{(b)}) uses the \code{lock} and \code{unlock} calls which represent 
accesses to an external resource.
The normal behavior is to \code{lock} the resource (ln. \code{(a)}:4 and 
\code{(b)}:3), perform some operations in subsequent reactions to input 
\code{A} (ln. \code{(a)}:5--8 and \code{(b)}:7--10), and then \code{unlock} the 
resource (ln. \code{(a)}:10 and \code{(b)}:5).
However, if the aborting input \code{B} (ln. \code{(a)}:12 and \code{(b)}:12) 
occurs after the \code{lock} but before the reactions to \code{A}, we still 
want to call \code{unlock} to safely release the resource.
%
In Esterel and \CEU, the finalize clauses (ln. \code{(a)}:10 and \code{(b)}:5) 
are automatically called if the enclosing blocks (ln. \code{(a)}:3--1 and 
\code{(b)}:3--10) are externally aborted (ln. \code{(a)}:12 and \code{(b)}:12).
