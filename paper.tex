...
4 radio messages, given that \emph{I/O} components are preloaded in the remote
devices.
In contrast, a \C-compiled version occupies more than 2 Kbytes, which includes 
all \emph{I/O} functionality~\cite{ceu.terra}.
...
loop do
    par/or do
        await e;
    with
        emit e; // w/o the restriction, awakes 1st trail and restarts the loop instantaneously
        await f;
    end
end
\end{lstlisting}
...
left of Figure~\ref{lst.fin}, which does not compile.
