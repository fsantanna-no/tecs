...
The Terra VM instruction set includes operations to support the \CEU execution 
model.
...
Plummer et al.~\cite{esterel.vm} present a reactive virtual machine for 
embedded systems.
This VM uses the Esterel programming language as scripting language and implements
a specific instruction set to the Esterel concurrent and multi-thread semantic.
They focus in code reduction instead of on efficiency.
Experimentally, they found the bytecode was about half the size of optimized
native assembly code generated from C.
The Terra virtual machine also implements a customized instruction set for a reactive
programming model, but it uses the \CEU model for concurrence and event 
control.
The Terra component based approach also contributes to have a very small bytecode size.
In our experiments, depending of the abstraction level of the pre-built customized components,
the bytecode size reduction may reach about 99\%.
