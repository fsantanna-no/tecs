...
In Section~\ref{sec.ceu.discussion}, after introducing internal events and 
...
control purposes (the compiler can restrict the available symbols in \C).
...
\end{lstlisting}
\centering\small{\ax Definition and use of external symbols}
\end{minipage}
%
...
.
\end{lstlisting}
\centering\small{\bx Annotations for the external symbols in \ax.}
\end{minipage}
%\rule{8.4cm}{0.37pt}
...
We extend the temporal analysis of \CEU for calls and accesses to external 
symbols in \C.
%
...
\code{\_NUM}.
%
Since \CEU does not inspect any code defined in \C, it complains about 
...
Esterel's \code{abort} and \CEU's \code{par/or} statements distinguish how 
...
TODO: queue-based/unique in the semantics of the language and not in the 
library

permeates all aspects

\CEU aims to offer a simpler semantics than Esterel while keeping the power of 
synchronous languages, with a tractable concurrency model, reactive and 
deterministic execution, and safe support for orthogonal abortion of lines of 
execution.
...
With the event-based semantics, \CEU does not impose a notion of simultaneity 
for events, requiring each application to define it only when necessary.
%
