...
intra-reaction execution which is amenable to a simple temporal analysis to 
improve safety:
%
...
resources (Section~\ref{sec.ceu.c}).
%
...
events (including zero), depending on external stimuli the environment 
provides.
...
time is discrete, its corresponding reaction only executes inthe next tick.
%
...
%
\item \code{Box-2}:
Esterel with variable-length ticks that adjust to the CPU time the 
...
%
\item \code{Box-3}:
\CEU with unique and queue-based policy for input events.
...
%
However, \CEU does not consider the events \code{B} and \code{C} simultaneous, 
which are handled in subsequent reactions \code{R(B)} and \code{R(C)}.
%
