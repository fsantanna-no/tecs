...
Internal events define fine-grained micro reactions which stack one on top of 
another inside the reaction to an external event.
...
(ln.  5, 8).
%
In the example \ax, the two assignments to \code{x} execute from reactions to 
different events \code{A} and \code{B} which, by definition, cannot occur 
...
%
However, an (apparently innocuous) change in the order of trails changes the 
...
within reactions.
Otherwise, any two trail segments in parallel can be concurrent, even if they 
...
The temporal analysis of Section~\ref{sec.ceu.analysis} also considers calls 
and accesses to external symbols in \C.
%
...
the first trail calls symbol \code{\_f},
...
compiler about the semantics of the \C symbols in use in \ax.
With the annotations in \bx, the program in \ax compiles successfully:
%
\begin{itemize}
...
From our experience, however, we find that it is not uncommon to require 
stateful and non-commutative calls to execute concurrently.
%
This is the case for logging (e.g., calls to \code{\_printf} in parallel) or 
redrawing objects in the screen.
%
Figure~\ref{lst.redraw}.a shows an abstract code to animate and redraw objects 
\code{back} and \code{fore} in the screen in trails in parallel.
In typical graphical APIs, consecutive calls to \code{\_redraw} overwrites 
conflicting pixels.
This way, \code{\_redraw} is non commutative and the code does not compile 
correctly.
%
However, since the \code{back} object should be redrawn before the \code{fore} 
object, we want to rely on \CEU's intra-reaction deterministic execution based 
on lexical order.
Therefore, in Figure~\ref{lst.redraw}, we redefine the redrawing function to an 
``intimidating'' name \code{\_redraw\_non\_commutative} to be explicit about 
its effect (ln. 2), and annotate it as safe (ln. 4--5) to compile successfully.

\begin{figure}
\begin{minipage}[t]{0.45\linewidth}
\begin{lstlisting}
par do
    <...> // animate and redraw "back"
        _redraw(back);
with
...
par do
    <...> // animate and redraw "back"
        _redraw_non_commutative(back);
with
    <...> // animate and redraw "fore"
        _redraw_non_commutative(fore);
end
\end{lstlisting}
...
Esterel's \code{abort} and \CEU's \code{par/or} statements distinguish 
synchronous from asynchronous languages with respect to orthogonal abortion of
lines of execution (i.e., without tweaking them with synchronization 
primitives)~\cite{esterel.preemption}.
In contrast, traditional (asynchronous) multi-threaded languages cannot express 
thread termination safely~\cite{sync_async.threadsstop}.
%
...
The example in Figure~\ref{lst.fin} in Esterel \ax and \CEU \bx uses the 
\code{lock} and \code{unlock} calls which represent accesses to an external 
resource.
...
\bx:5).
However, if the aborting input \code{B} (ln. \ax:12 and \bx:12) occurs after 
the \code{lock} but before the reactions to \code{A}, we still want to call 
\code{unlock} to safely release the resource.
%
In Esterel and \CEU, the finalize clauses (ln. \ax:10 and \bx:5) are 
automatically called if the enclosing blocks (ln. \ax:3--1 and \bx:3--10) are 
...
However, the ``sensation of simultaneity'' is not infinitesimal, but increases 
with the inaccuracy of the observer (e.g., a human being).
Therefore, simultaneity should be defined with respect to the observer, which 
...
which is a huge amount of time for a language-defined tick, and which would 
break the synchronous hypothesis.
%
Note that a similar implementation for Esterel would not rely on the tick 
...
\CEU aims to offer a deterministic and simpler semantics than Esterel with a 
more fine-grained control over the program execution.
%
The list that follows enumerate the contributions of our design in this 
direction:
%
\begin{itemize}
%
...
development.
We believe that programmers are more familiar with this notion of time, which 
simplifies the reasoning about concurrency.
...
Determinism in \CEU is all-inclusive and does not depend on additional levels 
of static analysis.
It encompasses the whole language, including memory accesses, system calls, and 
the expressive stack-based internal events.
For instance, programmers can always figure out which statement executes next, 
making runtime analysis and debugging easier.
%
...
suspicious statements reacting concurrently to the same event.
This way, the complier forces the programmer to serialize the statements 
accordingly.
%
\item Safe integration with \C.
\CEU forces the programmer to deal with pointers representing resources through 
...
of accumulating timers in sequence and comparing timers in parallel.
%
\end{itemize}
...
%
\begin{itemize}
\item Execution is purely reactive as a result of event-triggered reactions.
...
As a synchronous language, \CEU requires that CPU times for reactions are 
...
The deterministic semantics of \CEU does not make parallelization easy (to be 
discussed in Section~\ref{sec.impl.single}).
\end{itemize}
%
Nonetheless, we advocate for a tractable synchronous reactive core that 
supports shared memory with deterministic execution.
%
To deal with the limitations, we recommend for memory-isolated parallelizable 
asynchronous primitives as language extensions, but which are not in the scope 
of this paper.
...
in comparison to hand-crafted event-driven code in \emph{nesC}~\cite{wsn.nesc} 
(a \C variant) available in a stable codebase%
...
The small increase in resource usage shows that \CEU fits the context of 
constrained embedded systems which is typically reserved to \C only.
...
- TODO: no available Esterel compiler for the constrained target platform
    - we compare with C, which is a benchmark, technically unbeatable for 
      resource efficiency
...
%
\item An \code{await E} statement has $O=\{E\}$, where \code{E} is an input 
event (see internal events below).
%
...
$I_n=O_{n-1}$.
The first child inherits $I$ from its parent node, while the set $O$ for the 
whole node is copied from its last child, i.e., $O=O_n$.
%
...
TODO: most about comparison with Esterel
    - TODO: related work?
    - As far as we know, \CEU is the first Esterel-based language to HAVE?
      queue-based/unique in the semantics of the language and not in the
      library
...
% SIMPLE
    ...
    - flexible, coroutines, resumable exceptions
    - yet simple, programmers understand stacks
    - most of the time doesn't matter

Intra-reaction determinism is another
The temporal analsysi (Section~\ref{sec.ceu.analysis}).


The temporal analsysi (Section~\ref{sec.ceu.analysis}).
simple, programmers understand

targets control-intensive applications and provides abstractions that can 
express program flow specifications concisely.

lexical order pragamatics
    - again, familiar to programmers
        - register order
    - implementation aspects in Section XX

TODO: move POE to here, simpler more natural for software dev

- static analsysis also simpler
    - highlight that is not only important to have a comptutally-sound 
      algorithm, but that something that programmers can understand
- same with 1st-class timers, also, too much common to be ignored
...
Esterel XXX
thanks to the uniqueness of events, pointer tracking, and first-class timers, 
\CEU also provides additional safety protections:
%
No concurrency in accesses to shared variables and system calls sharing a 
resource
Finalization for blocks going out of scope
Auto-adjustment for timers in sequence and synchronization for timers in parallel
...
TODO: search "limitation" in the paper and condense all
...
regulary on every tick.
%
Although it supports asynchronous active execution~\cite{ceu.tr}, we still 
consider \CEU as purely reactive since its synchronous core cannot express 
active behavior.
