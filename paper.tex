...
%
\CEU uses an event-triggered notion of time that enables compile-time concurrency 
...
stack-based internal events, the concurrency analysis, safe integration with \C, 
...
execution, and is amenable to a concurrency analysis that
improves safety.
%
...
%
\item Static concurrency analysis to detect suspicious concurrent statements.
...
purpose of \CEU's \code{par/or} (ln. 4--13):
...
prerequisite for the concurrency analysis to be discussed in 
Section~\ref{sec.ceu.analysis}.
\end{itemize}
...
\subsection{ Static Concurrency Analysis }
...
To mitigate this threat, \CEU performs a concurrency analysis at compile time to 
...
As a safety measure, the concurrency analysis of Section~\ref{sec.ceu.analysis} 
...
%
\item \emph{Static concurrency analysis.}
...
to improve the static analysis results, or to force non commutative 
concurrent behavior.
...
core~\cite{esterel.crp} (which are in the scope of this paper).
...
tree (\emph{AST});
the \emph{concurrency analysis phase} detects inconsistencies in programs, such as 
...
\CEU:
concurrency analysis for determinism (Section~\ref{sec.impl.analysis}),
...
The compile-time concurrency analysis phase detects inconsistencies in \CEU 
programs.
...
\CEU favors a fine-grained use of trails: it is common to use trails that await 
...
Note that, as discussed in Section~\ref{sec.ceu.analysis}, the concurrency 
...
%(similar to \CEU).
%
\CEU goes one step further and performs a concurrency analysis to detect trails 
...
%prevents the concurrency analysis of \CEU.
%
