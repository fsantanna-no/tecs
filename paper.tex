...
one aiming for efficiency and interoperability with \C,
...
\C, and observed a reduction in source code size (around 25\%) with a small 
increase in memory usage (around 5--10\% for ROM and RAM)~\cite{ceu.sensys13}.
%
Considering the \VM backend, a simple application that blinks three LEDs 
periodically occupies less than 100 bytes and can be completely transmitted in 
4 radio messages.
In contrast, without the preinstalled \VM with basic \emph{I/O} functionality, 
a \C-compiled version occupies more than 2 kbytes~\cite{ceu.terra}.
...
\CEU.
Section~\ref{sec.impl} presents the \C and \VM implementation backends.
...
the set of customized pre-built components,
...
\begin{figure}[!htb]
 \centering
 \includegraphics[width=0.7\columnwidth]{terra}
\caption{Terra programming system basic elements.}
 \label{fig.terra}
\end{figure}

\CEU scripts in Terra cannot execute arbitrary \C code, instead, they rely on 
pre-built components that can be customized for different application domains.
%
Considering the domain of WSNs, Terra already provides components organized in 
four areas: radio communication, group management, data aggregation, and local 
operations (e.g., access to sensors and actuators).
%
When creating an instance of the \VM, the programmer can choose whether or not 
to include each component, setting different abstraction boundaries.
%
The generated \VM has to be preloaded into the embedded devices before they are 
physically distributed.

The communication between \CEU and the Terra components is mostly through 
events:
scripts make requests through \code{output} events and \code{await} answers 
through \code{input} events.
%
Terra also provides system calls for initialization and configuration of 
components.

TODO: exemplo de implementacao/uso de um componente
...
the \C backend, the main difference resides in the \emph{Code Generation}, 
which now outputs assembly instructions for the Terra \VM, instead of \C.

TODO: falar um pouco do assembly/bytecode, quais as decisoes envolvidas
- (analogo a Figura 18)
