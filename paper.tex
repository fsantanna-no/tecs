...
messages.
In contrast, the full image of a \C-compiled version occupies more than 2 
Kbytes~\cite{ceu.terra}.%
\footnote{
The \VM is preloaded in the devices with some \emph{I/O} functionality, whereas 
\C-based binaries are full images.
}
...
\CEU is a synchronous reactive language based on Esterel with support for 
...
\code{abort-when} (ln. 3--10), which serves the same purpose of \CEU's 
\code{par/or} (ln. 3--12):
...
sequences, loops, parallels, and also assignments.
...
do not fit the zero-delay hypothesis, and cannot be elegantly implemented.
...
Esterel and \CEU.
%
In the version in Esterel, when \code{A} occurs, \code{B} is emitted (ln.  
...
%
\CEU imposes a restriction that an \code{await} is only valid for the next 
reaction, i.e., if an \code{await} and \code{emit} occur simultaneously in 
parallel trails, the \code{await} does not awake.
%
...
loop do
    par/or do
        await e;
    with
        emit e;     // w/o the restriction, the emit awakes 1st trail
...
In this case, moving the \code{await} to the end of the loop (ln. 10) makes the 
...
state variable (ln. 2) and also to copy the condition test to the periodic code 
(ln. 13).
%
...
Embedded applications make extensive use of global memory, such as for 
accessing resources through memory-mapped registers.
...
\code{x} (ln. 5,8) can never run concurrently, because each trail segment 
reacts to a different input event (ln. 4,7) and, according to the semantics of 
\CEU, cannot occur simultaneously.
%
However, the program in the right is non-deterministic, because the two 
assignments to \code{y} (ln. 5,8) occur in the same reaction to input \code{A} 
(ln. 4,7).
...
left of Figure~\ref{lst.fin}, which does not compile in \CEU.
It describes the state machine of a data collection protocol for sensor 
networks~\cite{wsn.ctp,ceu.sensys13}.
%
The input events \code{STOP}, \code{RETRANSMIT}, and \code{SENDACK} (ln. 1) 
...
one simply monitors the stopping event (ln. 3);
the other periodically transmits the packet (ln. 5--18).
%
The periodic transmission is a loop that starts two other trails (ln. 6--17):
one handles the immediate retransmission request (ln. 7);
the other transmits the packet and waits for a confirmation (ln. 9--16).
%
%
The actual transmission (ln. 12--15) is enclosed with a \code{par/and} that 
takes at least one minute before looping (ln. 10), in accordance with the 
specification.
Note that the call to \code{\_send\_enqueue}%
\footnote{
...
(ln. 14) is \emph{asynchronous}, handing to the radio driver a pointer to the 
lexically-scoped packet (ln. 12).
...
(ln. 15).
%
At any time, the client may request a retransmission (ln. 7), which terminates 
the \code{par/or} (ln. 6), aborts the ongoing transmission (ln. 14, if not 
idle), and restarts the loop (ln. 5).
%
The client may also request to stop the whole protocol at any time (ln. 3).
%
...
expressions are equivalent to their counterparts in the concrete language.
...
identifying the current reaction, maps a program $p$ and a stack of events $S$ 
in a single step to a modified program and stack:
%
\begin{align*}
\LL S, p \RR &\ST
\LL S', p' \RR
...
expressions.
The concrete code in the left communicates the value \code{1} from the 
\code{emit} to the \code{await}.
The abstract code uses a shared variable to hold the value.
...
In the concrete program in the left, the variable \code{dt} holds the residual 
delta time (as described in Section~\ref{sec.ceu.timers}) after awaking from 
the timer.
...
original body in parallel with a $fin$ expression to run the finalization code.
...
suspicious accesses to shared memory, and also performs ``classical'' semantic 
analysis, such as building a symbol table for checking variable declarations.
%
For the (standard) \C back end, this phase outputs code in \C.
%
...
The code in the right of the Figure~\ref{lst.impl.trails} dispatches a trail 
segment according to the current label to execute.
%
...
\begin{description}
\item[{\textbf\code{ceu\_go\_init()}}:] initializes the program (e.g., the 
initial state of trails) and executes the ``boot'' reaction (i.e., the 
\code{Main} label).

\item[{\textbf\code{ceu\_go\_event(id,param)}}:] executes the reaction for the 
received external event id and associated parameter.
...
concurrently (i.e. while a reaction is running).
However, a binding must never interleave or run multiple API calls in parallel.
...
%
The left of Figure~\ref{lst.terra.defs} shows a \CEU interface with the 
available functionality for a customized \VM with temperature and radio 
components.
The right of the figure shows the associated binding for output events (ln.  
1--8), input events (ln. 10--14), and system calls (ln. 16--22).
%
Note that all applications for the customized \VM must comply with the same 
interface.
In contrast, the template-based \C back end allows applications to choose all 
possible combinations of functionalities from the underlying platform at 
compile time (as illustrated in Figure~\ref{lst.impl.tinyos}).
...
output int  REQUEST_SEND;    // sends int

// Input events
input int  TEMPERATURE_DONE; // receives int
...
\caption{
%{\small %\textmd{
\CEU interface (in the left) with associated customized \VM (in the right).
%
\code{VM.out} redirects all output events to the corresponding OS call (ln.  
1--8).
%
...
\C.
%
The \VM includes special instructions to support complex and recurrent 
operations from the runtime of \CEU, such as dispatching events, clearing 
aborted trails
Basically, they map
...
%
The \emph{Sun SPOT} Java-based platform with the \emph{Squawk 
JVM}~\cite{wsn.sunspot} brings Java for the embedded domain, but requires a 
...
To reduce code size, the \VM has a instruction set to deal with event handling 
and concurrency constructs that are particular to Esterel.
