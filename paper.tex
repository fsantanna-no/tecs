...
\begin{abstract}
\CEU is a synchronous language inspired by Esterel with simpler semantics and 
more fine-grained control over program execution.
%
\CEU adopts an event-triggered notion of time which enables compile-time 
analysis for simultaneous reactions to produce deterministic and 
concurrency-safe programs.
%
Using Esterel as a base, we discuss the particularities of our design, such as 
...
%
In \CEU, instead of clock ticks, atomic external event occurrences define time 
units.
%
...
%
As far as we know, \CEU is to encrust this event-triggered notion of time in 
the core of the language, which is a prerequisite for the temporal analysis 
...
%(similar to \CEU).
%
\CEU goes one step further and performs a temporal analysis to detect trails 
that, when reordered, change the observable behavior of the program, i.e., 
trails that actually rely on deterministic scheduling.

%
%These languages have a tick-based notion of time similar to Esterel, which 
%prevents the temporal analysis of \CEU.
%
\CEU also guarantees deterministic behavior for timer compositions by adjusting 
inaccuracies in the system clock.
...
enforce the programmer to provide associated finalizers.
