...
Another distinction regarding event handling is that Esterel supports 
same-cycle bi-directional communication~\cite{esterel.compiling}, i.e., two 
threads can react to one another during the same cycle due to mutual signal 
dependency.
%
\CEU imposes a restriction that an \code{await} is only valid for the next 
reaction, i.e., if an \code{await} and \code{emit} for the same event occur 
...
%
The example of Figure~\ref{lst.delay} relies on this restriction to avoid 
infinite execution:
...
%
In an atypical scenario requiring immediate awake, delayed awaits can be 
circumvented by judiciously copying the code to execute on awake.
...
\end{lstlisting}
\caption{ The \code{await} occurs simultaneously with the \code{emit} and does 
not awake, preventing the \code{par/or} to terminate and execute the 
\code{loop} indefinitely.
\label{lst.delay}
}
\end{figure}
