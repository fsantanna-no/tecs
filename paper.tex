...
\CEU aims to offer a simpler semantics than Esterel without giving up its 
tractable synchronous concurrency model:
...
%
\begin{itemize}
\item \textbf{Event-triggered notion of time bound to the semantics of the 
language}, which allows ....
\item \textbf{Deterministic and fine-grained intra-reaction execution and 
communication}, lexical, stack-based internal events
\item \textbf{Static temporal analysis for concurrent statements}, which
      prevent logical race conditions on shared variables and system calls.
\item \textbf{Safe integration with \C}, with enforced resource clean up.
\item \textbf{First-class timers} with dedicated syntax and automatic 
synchronization.
\end{itemize}
...
and extend it with memory-isolated parallelizabe asynchronous execution.
    ...
    - imposed intra-reaction execution
        - for inter-reaction commm
        - for intra-reaction commm
        - for stmts
