...
original version in \emph{nesC} (a dialect of \emph{C} for event-driven 
...
defines a time unit.
The event-driven approach is popular in many 
communities~\cite{TOS,games,GUI,callbacks,nodejs}, and is essentially 
synchronous, even though it is not commonly/formally 
noticed/acknowledged/recognized as such.
(TODO: rewrite)
%
...
%
Finally, \CEU also distinguishes external events from stack-based internal 
events, which provides a limited form of coroutines that supports \CEU's 
reactive statements.
