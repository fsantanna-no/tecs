...
\begin{abstract}
\CEU is a synchronous language inspired by Esterel with support for a simpler 
semantics and more fine-grained execution control.
%
\CEU employs an event-triggered notion of time which enables compile-time 
analysis for simultaneous reactions to produce deterministic and 
concurrency-safe programs.
%
We discuss the particularities of our design in comparison to Esterel, such as 
stack-based internal events, temporal analysis for concurrent statements, safe 
integration with \C, and first-class timers.
%
...
requires careful static analysis such as to detect and refuse programs with 
...
threads to interact with stateful system calls safely and makes shared-memory 
concurrency not as straightforward as reading and writing to shared variables.
%
...
%
% TODO: simple simple
Overall, \CEU has a simple semantics with fine-grained control for 
intra-reaction execution which is amenable to a simple temporal analysis that
improves safety.
%
...
%
\item Safe integration with \C which enforces finalization for external 
resources.
...
}, showing the practical aspect of our simple semantics.
...
and device drivers.
%
We evaluated the expressiveness of \CEU in comparison to hand-crafted 
event-driven code in \C and attested a reduction in source code size (around 
...
Figure~\ref{lst.abro} shows side-by-side the implementations in Esterel \ax and 
\CEU \bx for the following control specification:
%
...
\code{abort-when} (ln. 4--11), which, in this case, serves the same purpose of 
\CEU's \code{par/or} (ln. 4--13):
...
In the subsections that follow, we discuss the main differences between \CEU 
and Esterel:
%
...
Stack-based internal events
    (\ref{sec.ceu.ints});
Static temporal analysis for concurrent statements
    (\ref{sec.ceu.analysis});
Safe integration with \C
    (\ref{sec.ceu.c});
and
First-class synchronized timers
    (\ref{sec.ceu.timers}).
%We finish with a summary of our design
    %(\ref{sec.ceu.discussion}).
%
We provide the formal specification of the semantics of \CEU in a separate 
work~\cite{ceu.phd}.
...
events, depending on external stimuli the environment provides.
...
\end{enumerate}
%
A program must react to an occurring event completely before handling the next.
%
...
running, it is enqueued to occur in the next reaction.
...
The other boxes define distinct discrete logical units of time in which the 
same occurring events fit differently.
%
\begin{itemize}
\item \code{[Box-1]}:
``Esterel with fixed-length ticks'',
within which reactions must fit~\cite{esterel.fixed}.
%
We assume a \code{R(boot)} reaction at \code{tick-0} which happens before any 
input.
%
...
%
Note that \code{R(A)} takes more time than \code{tick-1}, causing a 
\emph{timing violation}~\cite{esterel.fixed} that invades \code{tick-2}.
%
...
However, only one occurrence of \code{E} is considered in \code{R(D+E)}.
%
\item \code{[Box-2]}:
``Esterel with variable-length ticks'', which are adjusted to the CPU time the 
corresponding reaction takes to complete~\cite{esterel.variable}.
This approach avoids the time violation for \code{R(A)} and also results in
smaller idle periods.
For instance, the occurrence of \code{D} interrupts the idle \code{tick-3} to 
react alone as \code{R(D)} on \code{tick-4}.
Similarly to the fixed-tick approach, only one of the two simultaneous 
occurrences of \code{E} is considered on \code{tick-5}, now because \code{R(D)} 
takes too long.
%
\item \code{[Box-3]}:
``\CEU with unique and queue-based input events''.
We also assume a \code{R(boot)} reaction before any input.
%
Because the occurrence of event \code{A} is unique during \code{tick-0}, the 
behavior in \CEU is similar to \code{Box-2} for the first two units of time 
(\code{boot-0} and \code{A-1}).
%
...
We decided for the unique and queue-based semantics in \CEU for the reasons 
that follow:
%
\begin{itemize}
...
in the real world.
Also, since ticks require no time regularity~\cite{esterel.multiclock}, the two 
approaches for Esterel in Figure~\ref{fig.ticks} are legitimate, but lead to 
different behaviors for the same sequence of inputs.
%
...
the language, but defined explicitly for each use case.
In the case of Esterel, simultaneity depends on the imprecise length of 
discrete ticks.
For instance, in Figure~\ref{fig.ticks} (\code{box-1} and \code{box-2}), the 
events \code{B} and \code{C} are considered simultaneous, even though \code{A} 
and \code{B} ``physically'' happen much closer to one another.
In Section~\ref{sec.ceu.timers}, after introducing internal events and 
first-class timers, we show an example to detect simultaneous button clicks in 
\CEU.
%
\item \textbf{Unique input events imply mutual exclusion:}
Reactions to multiple events never overlap because they are atomic.
Automated mutual exclusion simplifies the reasoning about the program and is a 
prerequisite for the temporal analysis to be discussed in 
Section~\ref{sec.ceu.analysis}.
\end{itemize}
...
occurrence and actual reaction of a given event.
%
...
accumulates for subsequent ticks.
%
...
% TODO-no: Such analsysis is orthogonal

A limitation of event-triggered execution is that all program behavior must be 
...
regulary on every tick.
%
Although \CEU supports asynchronous active execution~\cite{ceu.tr}, we still 
consider it as purely reactive since its synchronous core cannot express active 
behavior.
...
Internal events provide a limited form of subroutines, as depicted in 
Figure~\ref{lst.sub}.
...
For instance, the code that executes after the \code{await inc} (ln. 4) could 
\code{emit} another event, creating a new level in the stack.
The runtime stack represents fine-grained micro reactions, one on top of the 
other, all inside the same reaction to an external event.
...
%
In a previous work, we build on top of internal events other advanced control 
mechanisms, such as resumable exceptions and reactive 
...
instantaneous termination (ln. 4,7) of the enclosing \code{loop} (ln 2--9).
...
%
A number of Esterel-based synchronous languages enforce intra-reaction 
determinism with an arbitrary execution order for statements in multiple lines 
of execution (%
...
which define a shared variable (ln. 2), and assign to it in parallel trails 
(ln.  5, 8).
%
In the example \ax, the two assignments to \code{x} only execute in reactions 
to different events \code{A} and \code{B} which, by definition, cannot occur 
simultaneously (Section~\ref{sec.ceu.exts}).
Hence, for the sequence \code{A->B}, \code{x} becomes \code{4=(1+1)*2}, while 
for \code{B->A}, \code{x} becomes \code{3=(1*2)+1}.
%
...
is still deterministic, and \code{y} always becomes \code{4=(1+1)*2}.
%
...
To mitigate this threat, \CEU performs a temporal analysis at compile time to 
detect concurrent accesses to shared variables:
...
%
\begin{itemize}
\item The assignments to \code{x} in lines 2 and 5 in \ax
      \textbf{cannot} be concurrent because they are \textbf{not} in parallel trails.
\item The assignments to \code{x} in lines 5 and 8 in \ax
      \textbf{cannot} be concurrent because they \textbf{cannot} execute during 
      the same reaction.
\item The assignments to \code{y} in lines 5 and 8 in \bx
...
Section~\ref{sec.impl.analysis}, inspects all possible \code{await} statements 
that precede a variable access and keep a list with all corresponding awaking 
events.
...
Note that such analysis is only possible due to the uniqueness of input events 
within reactions.
Otherwise, any two trail segments in parallel could be concurrent, even if they 
react to different input events.
%
Note also that the analysis is completely optional and does not affect the 
semantics of the program.
% TODO-no: static analsysis also simpler
...
As a safety measure, the temporal analysis of Section~\ref{sec.ceu.analysis} 
also considers calls and accesses to external symbols in \C.
%
...
The annotations in Figure~\ref{lst.c}.b provides hints to the compiler about 
the semantics of the \C symbols in code \ax, which now compiles successfully:
%
\begin{itemize}
...
From our experience, we find that it is not uncommon to also require 
non-commutative calls to execute concurrently.
%
...
conflicting pixels, which makes it non commutative and prevents the code to 
compile.
%
However, we want to rely on trails lexical order to always redraw the 
...
about its effect), and annotate it as safe (ln. 4--5) to compile successfully.
...
language in comparison to asynchronous languages~\cite{esterel.preemption}.
...
%
However, aborting lines of execution that deal with external resources is still 
error prone because they may end up in an inconsistent state.
%
...
The normal behavior is to \code{lock} the resource (ln. \ax:4 and \bx:3), 
perform some operations in subsequent reactions to input \code{A} (ln.  
\ax:5--8 and \bx:7--10), and then \code{unlock} the resource (ln.  \ax:10 and 
\bx:5).
...
%
In Esterel and \CEU, the finalize clause (ln. \ax:10 and \bx:5) execute
...
for expiring awaits.
The \CEU scheduler will notice that the \code{await 10ms} (ln. 2) has not only 
...
\CEU also considers that time is a physical quantity that can be added and 
compared.
...
synchronous model, the first trail (ln. 2--5) is guaranteed to terminate before 
the second trail (ln. 7--8), because $10+1 < 12$.
%
...
In Section~\ref{sec.ceu.exts}, we argue that events occurrences are 
...
Therefore, we consider that simultaneity should be defined with respect to the 
observer, which varies from case to case.
%
...
%
Therefore, a similar implementation for Esterel would not rely on the tick 
...
fine-grained control over the program execution.
%
The list that follows summarizes the contributions of our design in this 
direction:
%
\begin{itemize}
%
\item Event-triggered notion of time bound to the semantics of the language.
...
temporal analysis of \CEU.
%
\item Deterministic intra-reaction execution and communication.
...
%
\item Static temporal analysis for concurrent statements.
Although execution is deterministic, the compiler of \CEU still advises about 
suspicious statements that can react concurrently to the same event, forcing 
the to serialize them accordingly.
%
\item Safe integration with \C.
\CEU enforces the programmer to deal with pointers representing resources 
through finalization clauses.
When dealing with concurrent system calls, programmers can provide the compiler 
with annotations to remove false positives from the temporal analysis or to 
allow non commutative concurrent behavior.
%
\item First-class timers with dedicated syntax and automatic synchronization.
...
%
\begin{itemize}
\item Execution is purely reactive as result of event-triggered reactions.
Since only occurrences of events can start reactions, programs cannot execute 
proactively in the absence of events.
\item Reactions must execute in bounded time due to the synchronous hypothesis.
...
\item Execution is sequential because of intra-reaction determinism.
...
\end{itemize}
%
Nonetheless, we advocate for keeping a tractable synchronous reactive core 
which supports shared memory concurrency with deterministic execution.
%
To deal with the limitations above, we recommend for memory-isolated 
parallelizable asynchronous functionality as a separate extension to the 
synchronous core~\cite{esterel.crp} (which is not in the scope of this paper).
...
In a previous work~\cite{ceu.sensys13}, we evaluate the implementation of \CEU
in comparison to a hand-crafted event-driven code base%
\footnote{TinyOS repository: \url{http://github.com/tinyos/tinyos-release/}}
in \emph{nesC} (a \C variant)~\cite{wsn.nesc}.
%
Figure~\ref{fig.eval} compares source size (\emph{tokens}), binary size 
(\emph{ROM}), and memory usage (\emph{RAM}) for a number of standardized 
network protocols and a radio driver.
The small increase in resource usage shows that the gains in productivity and 
safety with \CEU make it a viable alternative to \C in the context of 
constrained embedded systems.

Unfortunately, most real applications in Esterel seems to be closed source.
In a future work, we plan to cooperate with groups that have more experience 
...
In the sections that follow, we discuss the implementation of subsystems that 
are specific (or in some way important) to \CEU:
...
The compile-time temporal analysis phase detects inconsistencies in \CEU 
programs.
...
input events $I$ (for \emph{incoming}) that can lead to the execution of the 
node, and also the set of input events $O$ (for \emph{outgoing}) that can 
terminate the node.
...
The code in Figure~\ref{lst.impl.ast}.a has the $AST$ and sets $I$ and $O$ for 
each node in Figure~\ref{lst.impl.ast}.b.
...
do have an intersection, they are not in parallel branches and are also safe.
...
\ax:3).
%
The \code{do-end} blocks in sequence (ln. \ax:2--7) are packed in a 
\code{union} (ln. \bx:2--3), given that their variables cannot be in scope at 
the same time, e.g., \code{MEM.a\_1} and \code{MEM.b\_2} (ln. \bx:2,3) can 
safely share the same memory slot.
%
...
%
A \code{STACKED} trail holds an associated numeric stack level and is delayed 
until the scheduler runtime reaches that level again.
%
...
to prevent from awaking in the same reaction it is reached.
%
...
adjusts them to properly handle overflows (actually, 2 bits to hold the 
...
simultaneous lines of execution with memory overhead of only 2\% in comparison 
to the original version in \C~\cite{ceu.sensys13}.
...
trails, e.g., the label \code{Awake\_e} will execute on \code{TRAIL-0} (ln.  
\ax:7).
%
...
and \code{break}), the compiler splits the trail in two segments with 
...
%
When the \code{Main} label reaches the \code{par/and} (ln. \ax:4), it stacks 
\code{TRAIL-1} (ln. \cx:4--7) and proceeds to the code in \code{TRAIL-0} (ln.  
\cx:9--14) to respect lexical execution order.
%
...
Note that, as discussed in Section~\ref{sec.ceu.analysis}, the temporal 
...
One reason is the continuous fork-and-rejoin overhead due to small reactions.
Another reason is the excessive locality of data due to stackless threads 
sharing contiguous static memory.
...
Single-thread dispatching may not be suitable for hard real-time activities.
In a previous work~\cite{ceu.sensys13}, we measure how synchronous lengthy 
...
hook functions in the API of the \CEU runtime whenever an external event 
occurs.
%
However, the binding must never interleave or run multiple API calls in 
parallel to preserve the sequential/discrete semantics of time in \CEU.
...
aspect of notion of time.
%
In \CEU, instead of clock ticks, the occurrence of a single external event 
defines an unit of time.
%
...
%
As far as we know, \CEU is unique in this notion of time encrusted in the core 
of the language, which is a prerequisite for the temporal analysis that enables 
safe shared-memory concurrency.

Another unique aspect of \CEU is the distinction between external and internal 
events.
Internal events define stack-based micro reactions within external reactions, 
...
%(similar to \CEU).
%
\CEU goes one step further and performs a temporal analysis to detect 
concurrent trails that actually rely on deterministic scheduling, i.e., the 
observable behavior of the program changes if they are reordered.
%
%These languages have a tick-based notion of time similar to Esterel, which 
%prevents the temporal analysis of \CEU.
%
\CEU also guarantees deterministic behavior for composition of timers by 
adjusting residual awaking times from inaccurate system clocks.
...
%
\CEU also tracks pointers representing resources that cross \C boundaries and
enforce the programmer to provide corresponding finalizers.

%\emph{URBI}~\cite{rp.urbi} is a dynamic reactive scripting language with a 
%rich set of control constructs for time management, event-driven 
%communication, and concurrency.
%Concurrency is based on dynamically created stackful coroutines, diverging 
%from our goals regarding resource efficiency and static bounds for memory and 
%execution time.
...
language inspired by Esterel with support for a simpler semantics and more 
fine-grained control for intra-reaction execution.
...
unique stack-based internal signaling mechanism.
%
